...
모든 명령에 `--org https://dev.azure.com/{ado_org}/`를 포함한다. **단일 명령**만 쓴다(cd·파이프·리다이렉션 없이 — 복합 명령이 필요하면 `.sh` 파일로 Write 후 `bash 파일.sh`로 실행). **조회 다이어트(필수)**: 조직 전체 덤프(`--project` 없는 `pr list`, `--top 1000`류) 금지, 조회는 `--query`로 필요한 최소 필드만 추출한다 — PR description 등 대형 JSON 전량 덤프가 컨텍스트를 태우고 회차를 워치독 상한까지 끌고 간다(2026-08-24 실관측).
- **PR 조회**: `az repos pr show --id {PR_ID} --org https://dev.azure.com/{ado_org}/ --query "{project:repository.project.name,repo:repository.name,repoId:repository.id,repoUrl:repository.remoteUrl,title:title,status:status,sourceRef:sourceRefName,targetRef:targetRefName,createdById:createdBy.id,createdBy:createdBy.displayName,sourceCommit:lastMergeSourceCommit.commitId,targetCommit:lastMergeTargetCommit.commitId}" -o json` (PR 본문 description은 리뷰 맥락에 필요할 때만 별도 단일 명령 `az repos pr show --id {PR_ID} --query "description" -o tsv`로 가져온다.)
- **PR 스레드 목록 조회**: `az devops invoke --area git --resource pullRequestThreads --route-parameters project={project} repositoryId={repo} pullRequestId={PR_ID} --org https://dev.azure.com/{ado_org}/ --api-version 7.1 -o json` → `value[]`가 스레드 배열(`comments[]`, `status`, `threadContext.filePath` 포함). 단, **멱등 판정(내 댓글 존재 확인)만 필요할 때는** `--query "value[?isDeleted != \`true\`].{id:id,status:status,author:comments[0].author.uniqueName}"` 로 충분하다 — 전체 본문은 타인 스레드 내용 검토가 실제로 필요할 때만 받는다.
...
- **레포 목록**(`search_code` 대체 후보 좁히기용): `az repos list --project {project} --org https://dev.azure.com/{ado_org}/ --query "[].{name:name,id:id}" -o json`. (`search_code`에 대응하는 az 명령은 없다 — 원본에서도 선택적 fallback이었으므로, 후보를 못 좁히면 원본 규칙대로 처리한다.)

## 1단계: 채널에서 새 리뷰요청 수집
- `slack_read_channel`로 채널 `{review_channel}`(#{review-channel})의 조회 구간 내 메시지를 **detailed 포맷**으로 읽는다 — 조회 하한은 아래 불변식대로 `oldest` 파라미터로 서버에 맡기고, 한 페이지를 넘길 만큼 많으면 페이지네이션으로 구간을 끝까지 소진한다. detailed 포맷의 메시지 텍스트는 mrkdwn 원문(예: 유저그룹 멘션이 `<!subteam^{SUBTEAM_ID}>` 같은 리터럴로 표기됨)이어야 한다 — 사람이 읽기 좋게 렌더링된 텍스트가 아니라 **원문 그대로**를 기준으로 다음 필터들을 적용한다.
- **조회 구간은 `lastRunAt - 10분` ~ `{runStartedAt}`** (메시지 TS 기준). watermark 자체는 `lastRunAt`이지만, 뒤늦게 수정되거나(edited) 직전 실행이 오판(예: 있어야 할 멘션을 놓침)으로 제외했을 가능성을 겹쳐서 다시 확인하기 위해 **10분을 겹쳐 스캔**한다. 겹치는 구간에서 중복 처리가 나지 않는 이유는 2단계의 "이미 내 댓글이 있음" 판정이 **PR 단위(Azure DevOps API 조회)** 로 멱등을 보장하기 때문이다 — **Slack 메시지 자체를 "이전에 봤음/제외했음"이라는 이유만으로 스킵하지 않는다.** 즉 직전 실행에서 이미 제외 판정한 메시지라도 이번 실행에서 처음 보는 것처럼 다시 판정한다. 구간 상한(`{runStartedAt}` 이후 메시지 제외)은 메시지 `ts`(epoch 문자열)를 `{runStartedEpoch}`와 **정수 비교**로 적용한다 — 날짜 문자열 파싱을 쓰지 않는다.
- **조회 방법 불변식 (2026-08-24 유실 사고 재발 방지):** `slack_read_channel`의 `oldest`는 **오직 epoch 산술로만** 만든다 — `oldest = max({lastRunTs} - 600, {runStartedEpoch} - 345600)` (10분 오버랩, 96시간 캡). epoch은 절대값이라 타임존 오류가 원리적으로 불가능하다. **ISO8601 문자열을 파싱해 epoch을 만드는 것을 금지**한다 — 실제 사고: `-u` 없는 `date -j -f`가 UTC 문자열 `lastRunAt`을 KST로 해석해 조회창이 9시간 과거로 밀렸고, "0건" 오판 뒤 watermark가 전진해 13:15의 리뷰 요청이 영구 유실됐다.
  - **`lastRunTs`가 없을 때(null/레거시 상태 파일):** `lastRunAt`(ISO)이 있으면 반드시 `date -u -j -f '%Y-%m-%dT%H:%M:%SZ' <값> +%s`처럼 **`-u`를 붙여 UTC로 1회 변환**해 쓰고(이번 실행의 3.5단계부터는 `lastRunTs`가 기록되므로 이 경로는 마이그레이션 1회용이다), `lastRunAt`조차 없으면 부트스트랩 값 `{runStartedEpoch} - 1200`을 쓴다.
  - `latest`는 넘기지 않는다(기본값 = 최신). 상한 필터는 위의 `ts` 정수 비교로 적용한다.
...
- `lastRunAt`을 **`{runStartedAt}`(맨 처음 캡처한 값)으로 그대로**, `lastRunTs`를 **`{runStartedEpoch}`(같은 순간의 epoch 캡처값)으로** 갱신한다 — 두 필드는 항상 같은 시각의 두 표현이어야 한다. 절대 이 시점에 "지금"을 다시 계산해서 쓰지 않는다 — 그러면 조회했던 구간 끝과 기록되는 watermark 사이에 갭이 생겨, 그 갭 사이 메시지를 다음 실행도 (10분 오버랩 폭보다 갭이 크면) 놓칠 수 있다.
...
- **0건 가드 (2026-08-24 유실 사고 재발 방지):** 단, 1단계에서 조회 구간 내 채널 메시지가 **한 건도 조회되지 않았다면**(필터로 전부 제외된 경우가 아니라 조회 결과 자체가 0건) `lastRunAt`·`lastRunTs`를 전진하지 **않고** 기존 값을 유지하며, 그 사실을 4단계 보고에 명시한다. 조기 게이트가 "새 채널 활동"을 보고 이 실행을 기동했는데 조회가 0건인 것은 자기모순 — 조회 쿼리 오류(타임존 등)일 가능성이 높고, 이때 watermark를 전진시키면 그 메시지가 10분 오버랩 밖으로 밀려 영구 유실된다. 조회 ≥ 1건이면(필터 제외만 있어도) 정상적으로 전진한다.
