...
- **0건 가드 (2026-08-24 유실 사고 재발 방지):** 단, 1단계에서 조회 구간 내 채널 메시지가 **한 건도 조회되지 않았다면**(필터로 전부 제외된 경우가 아니라 조회 결과 자체가 0건) `lastRunAt`을 전진하지 **않고** 기존 값을 유지하며, 그 사실을 4단계 보고에 명시한다. 조기 게이트가 "새 채널 활동"을 보고 이 실행을 기동했는데 조회가 0건인 것은 자기모순 — 조회 쿼리 오류(타임존 등)일 가능성이 높고, 이때 watermark를 전진시키면 그 메시지가 10분 오버랩 밖으로 밀려 영구 유실된다. 조회 ≥ 1건이면(필터 제외만 있어도) 정상적으로 전진한다.
