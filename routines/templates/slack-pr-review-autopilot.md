...
- **조회 방법 불변식 (2026-08-24 유실 사고 재발 방지):** `slack_read_channel`에 `oldest`/`latest` 같은 epoch 파라미터를 **넘기지 않는다** — 항상 최근 30개(부족하면 페이지네이션)를 받아 **메시지 `ts` 문자열을 조회 구간과 직접 비교**해 필터링한다. 부득이 시각을 epoch으로 변환해야 하면 반드시 **UTC로 파싱**한다(`date -u -j -f ...` — `-u` 없는 `date -j -f`는 UTC 문자열을 로컬(KST)로 해석한다). 실제 사고: `-u` 없이 변환한 epoch을 `latest`로 넘겨 조회창이 9시간 과거로 밀렸고, "0건" 오판 뒤 watermark가 전진해 13:15의 리뷰 요청이 영구 유실됐다.
