...
2. **새 댓글 확인**: `slack_read_thread`로 해당 스레드(`channel` + `messageTs`)를 읽어, `lastCheckedTs` **이후** TS의 댓글 중 **이 루틴이 단 답글(멱등 마커 포함)과 봇(`{qa_bot_slack_id}`) 메시지를 제외**한 새 댓글이 있는지 본다. 없으면 **항목의 `lastCheckedTs`를 이번에 읽은 스레드의 가장 최신 답글 TS로 전진시킨 뒤**(제외한 내 답글·봇 메시지가 최신이어도 그 TS까지 — 조기 게이트는 작성자를 구분하지 못하고 `lastCheckedTs` 이후 답글 존재만 보므로, 여기서 전진시키지 않으면 내 답글 하나가 매 10분 게이트 RUN을 영구 유발한다. 2026-08-20~24 실사고: 뒤늦게 게시된 봇 보류 답글 1건이 나흘간 매 회차 풀 기동을 일으킴) 항목 유지, 다음 항목으로.
