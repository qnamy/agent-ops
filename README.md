...
```
claude/
├── CLAUDE.md        # 전역 지침 정본 — Claude(~/.claude/CLAUDE.md)와 Codex(~/.codex/AGENTS.md)가 심링크로 공유
...
2. **도구 중립 코어 + 도구 전용 마킹** — 공통 지침은 그대로 두 도구에 적용되고, 한쪽에만 유효한 절은 `[Claude Code 전용]`처럼 명시해 다른 쪽이 무시한다.
