...
- **Two commands per unit, not one.** `orca worktree create --agent claude --prompt "..."` cannot set model or effort — `--agent <id>` takes an agent id, not a command line. Create the worktree first, then open a terminal that carries the flags:
  ...
  **Approval prompts**: `-a never` (`--ask-for-approval never`, or `approval_policy = "never"`) suppresses them, but **it breaks a harnie run**. Codex's `workspace-write` sandbox denies writes under `.git/` even inside the workspace, so `git add -A` fails where it writes a temporary object into `objects/`; `init` and even `runs` fail with `Operation not permitted`, because blocker derivation captures the tree too. Probed 2026-08-31: `os.tmpdir()` writes are allowed and relocating the git index does not help — the denial is the object database, not the index, and it reproduces in an ordinary repo as well as a linked worktree, so it is policy rather than a path mismatch. So: run harnie in Codex with approvals **on**, and expect one prompt per new command prefix. Opening `.git` through `writable_roots` in `~/.codex/config.toml` would work but deliberately unlocks what the sandbox is protecting — not recommended. Choosing "always allow" reduces but does not eliminate them — some prompts offer no such option (condition not identified). Reserve `-a never` for sessions that do not drive harnie.
