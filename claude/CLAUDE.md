...
- **Small development work in an Orca workspace [Claude Code only]** → the user creates the worktree and workspace before starting `dev` or `dev-solo` there. harnie 0.14 accepts the run root and no longer supplies the previous incidental worktree safety net.
