...
> **언어 정책**: 이 레포의 영문 문서(`*.md`)가 실행 정본이고, 각 문서는 사람이 읽기 위한 한국어 미러(`*-ko.md`)를 쌍으로 유지한다. **정본 지침을 수정하면 같은 변경에서 `-ko.md` 미러도 갱신한다**(내용 동등성 유지 — 두 버전이 어긋나게 두지 않는다). 충돌·모호 시 영문 정본이 우선한다. (이 파일은 CLAUDE.md의 한국어 미러다.)
...
- **자동승인 받을 Bash 명령에 셸 확장·복합명령을 쓰지 않는다.** `; echo $?`, `&& echo ...`, `$?`·`$(...)`·`` `...` `` 같은 확장이 들어가면 보안 가드("Contains simple_expansion / command_substitution" 등)에 걸려 allow 규칙으로도 자동승인이 안 되고 매번 프롬프트가 뜬다. (전역 `settings.json`의 PreToolUse 훅 — `agent-ops/scripts/hook-bash-guard.py` — 은 인용부호를 인식한다: **인용부호 밖의** `&&`·`;`·`||`·명령치환·백틱만 자동 차단한다. 작은따옴표 안의 연산자, 큰따옴표 안의 `;`·`&&`·`||`는 리터럴이므로 통과한다 — 예: `jq 'test("a;b")'`, `rg 'foo;bar'`는 안전. 단 `$(...)`와 백틱은 큰따옴표 안에서도 실제 실행되므로 차단된다. 진짜 복합 명령은 단일 명령 여러 개로 나누거나 `.sh` 파일로 Write 후 `bash 파일.sh`로 실행한다.)
...
- **부분 읽기**: 큰 파일은 `rg`(또는 Grep)로 위치를 좁힌 뒤 offset/limit으로 해당 구간만 Read한다 — 또는 `rg -n -C <n>`으로 필요한 구간만 한 번에 뽑는다. 전체 Read는 파일이 작거나 전체 구조 파악이 꼭 필요할 때만.
- **재읽기 금지**: 같은 세션에서 이미 읽은 파일을 다시 읽지 않는다. Edit/Write 직후 확인 목적의 재읽기도 하지 않는다(실패하면 툴 에러로 드러난다).
- **탐색은 좁혀서**: 파일을 하나씩 열어보며 찾지 않는다. `rg`/Glob으로 후보를 좁힌 뒤 필요한 것만 읽는다.
- **Grep 도구 대신 `rg` [Claude Code 전용]**: Bash를 쓸 수 있는 곳(메인 세션)에서는 검색을 레포 루트 기준 **상대경로** 단일 `rg` 명령으로 실행한다. 엔진은 같은 ripgrep이지만 Grep 도구 결과는 출력 줄마다 절대경로 프리픽스(깊은 worktree에서 ~70자+)와 헤더 라인이 붙고, 상대경로 `rg` 출력에는 없다. `Bash(rg *)`가 허용 목록에 있어 프롬프트 비용도 없다. 전역 PreToolUse 훅(`agent-ops/scripts/hook-grep-guard.py`)이 메인 세션 Grep을 거부해 이를 강제하며, Bash 없는 서브에이전트(read-only 에이전트)와 `.harnie` 경로는 Grep을 그대로 쓴다.
