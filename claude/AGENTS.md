...
> **Canonical**: `~/workspace/agent-ops/claude/AGENTS.md` (repo `qnamy/agent-ops`). `~/.codex/AGENTS.md` (Codex) symlinks to it; `~/.claude/CLAUDE.md` (Claude Code) symlinks to `claude/CLAUDE.md`, which is one `@AGENTS.md` import line. Edit this file, never the stub. Sections marked **[Claude Code only]** do not apply in Codex.
...
- **Large development dispatch [Claude Code only]** → the design's section 3 병렬 단위 (harnie `software-design`, checked by `software-design-review`) fixes each unit's file ownership and order; choose each unit's model and effort per `delegation.md`; then execute through `~/workspace/agent-ops/claude/orca-dispatch.md` §Chain units from a design. A small bounded change with no split goes to harnie `dev` instead, which runs the chain on one instruction and refuses what is not small.
...
**Goal**: run shell commands in a shape the permission rules auto-approve, and inspect files the cheapest way.

**MUST**

- Read a hook's deny message and comply with the fix it carries (the only deny hook left is harnie `skill-guard`, which blocks the bundled `code-review`/`simplify` skills and names the chain skill to use instead). Compound commands are judged per subcommand by Claude Code itself; a part with no allow rule prompts, and that prompt is the signal to add the rule, not to split the command.
...
- **Narrow, then read**: locate with `rg` or the Grep tool, then Read only the needed range (`rg -n -C <n>` for a region in one step). Give Grep no path or a relative one: its output is then byte-identical to `rg` from the repo root, while an absolute path puts a ~50-character prefix on every line (measured 2026-09-08). Full reads only when the file is small or whole-structure understanding is genuinely needed.
...
2. **Simplicity first.** Overengineering is a defect: no speculative features, abstractions, or flexibility; defensive coding only at trust boundaries (external input, API, DB, network). **A runtime number or condition a mechanism turns on — a timeout, a retry count, an interval, a rate or concurrency limit, the error code it retries on, the state it skips on — stands on a measurement of this system, never on what is usually done**; an external reference can justify the mechanism but does not fix what it turns on, and a value the request itself names is no exception. Without that measurement, say so and ask rather than writing in the usual number. The harnie `software-design` §Minimum design and `implementation` skills carry the same rule. DRY/SOLID only after the rule of three; if 200 lines could be 50, rewrite — would a senior engineer call this overcomplicated?
